...
export class TimeoutCollection {
  ...
  public add(
    ...
    let id = originalSetTimeout.apply(global, [
      this._getWrappedHandler(newTimeout.uuid, handler),
      timeout,
      args,
    ]);
    newTimeout.id = id;
  ...
  private _getWrappedHandler(timeoutUuid: string, handler: Function): Function {
    return (...args: any[]) => {
      this._timeoutCollection[this._getTimeoutIndexByUuid(timeoutUuid)].status =
